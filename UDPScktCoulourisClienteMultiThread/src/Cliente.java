...
class ThreadCliente implements Runnable {
    ...
    protected String msg [] ={"1;5;VVFFV",
    		"2;5;VFFVV",
    		"3;5;VVFVV",
    		"4;5;FVVVF",
    		"5;5;VVFFV"};
    ...
    public void run() { // a interface Runnable exige a implementação do método run
        ...
        try {
            ...
            for(int q=0; q <5;q++){
                byte[] m = msg[q].getBytes(); // transforma arg em bytes
                InetAddress serv = InetAddress.getByName(servidor);
                DatagramPacket req = new DatagramPacket(m, msg[q].length(), serv, porta);
                s.send(req); // envia datagrama contendo a mensagem m
                System.out.println("* " + thread.getName() + " * Datagrama enviado: " + msg[q]);
                }
